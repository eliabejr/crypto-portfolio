<template>
  <div
    ...
    <div class="flex items-start justify-between mb-4">
      ...
      <button v-if="showWatchlistButton" :class="[
        'p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200',
        ...
        <svg v-if="localIsInWatchlist" :class="[
          'w-5 h-5 text-yellow-500 fill-current transition-all duration-300',
        ...
        <svg v-else :class="[
          'w-5 h-5 text-gray-400 hover:text-yellow-500 transition-all duration-300',
          isAnimating ? 'scale-125' : 'scale-100',
        ]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
...
const localIsInWatchlist = ref(cryptoService.isInWatchlist(props.crypto.id))
const isAnimating = ref(false)

const updateLocalState = () => {
  localIsInWatchlist.value = cryptoService.isInWatchlist(props.crypto.id)
}
...
const toggleWatchlist = async () => {
  ...
  try {
    await favoritesApi.toggleFavorite(props.crypto.id)
  } catch (error) {
    localIsInWatchlist.value = previousState
    if (previousState) {
      cryptoService.addToWatchlist(props.crypto.id)
